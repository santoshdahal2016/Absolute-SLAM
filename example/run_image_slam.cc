...
void mono_tracking(const std::shared_ptr<openvslam::config>& cfg,
    ...
    std::thread thread([&]() {
        for (unsigned int i = 0; i < frames.size(); ++i) {
            ...
            const auto img = cv::imread(frame.img_path_, cv::IMREAD_GRAYSCALE);

            const auto tp_1 = std::chrono::steady_clock::now();

            if (!img.empty() && (i % frame_skip == 0)) {
                // input the current frame and estimate the camera pose
                DATASETImage query_image = DATASETImage(frame.img_path_, frame.timestamp_);
                query_image.SetSlamPose(SLAM);

                // openvslam::Mat44_t cam_pose = SLAM.feed_monocular_frame(img, frame.timestamp_, mask);
                //  std::cout<<"Rotation = "<<query_image.SlamPose_co().GetRotmatrix()<<std::endl;
                // //    HomogeniousTF_t h_co;

                // if(SLAM.is_ok()){
                //  std::cout<<"Rotation = "<<cam_pose.block<3,3>(0,0)<<std::endl;
                //  std::cout<<"Translation = "<<cam_pose.block<3,1>(0, 3)<<std::endl;
